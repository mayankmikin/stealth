...
(function($) {
	...
	B.execute = function(g, h) {
		...
		var y = $("<option>" + g.warnNoMatch + "</option>").attr("disabled",
				"true");
		var z = {
			option : function(a) {
			...
			reset : function() {
				if ((m.get(0).selectedIndex - 1) == m.data("index"))
					return;
				...
				this.size(e - d);
				for (var i = d; i < e; i++)
					u.append($(m.get(0).options[i]).clone().attr(o, i - 1));
				if (e > g.maxMultiMatch)
					u.append(x);
		...
		u.mousemove(function(e) {
			if ($.browser.opera && parseFloat(jQuery.browser.version) >= 9.8)
				return true;
			var a = Math.floor(parseFloat(/([0-9\.]+)px/.exec(z.option(0).css(
					"font-size"))));
			var b = 4;
			if ($.browser.opera)
				b = 2.5;
			if ($.browser.safari || $.browser.chrome)
				b = 3;
			a += Math.round(a / b);
			z.selectedIndex(Math
					.floor((e.pageY - u.offset().top + this.scrollTop) / a))
		});
		r.click(function(e) {
			t.click()
		});
		t.keyup(function(e) {
			if (jQuery.inArray(e.keyCode, new Array(9, 13, 16, 33, 34, 35, 36,
					38, 40)) > -1)
				return true;
		...
		function draw() {
			...
			var c = parseFloatPx(m.css("padding-left"))
					+ parseFloatPx(m.css("padding-right"))
					+ parseFloatPx(m.css("border-left-width"))
					+ parseFloatPx(m.css("border-left-width")) + 23;
		...
		;
		function populate() {
			if (z.selectedIndex() < 0 || z.selected().get(0).disabled)
				return;
